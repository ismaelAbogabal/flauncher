...
class AppsService extends ChangeNotifier {
  ...
  /// check if mawaqit exists and lunch it
  Future<void> checkForAutoStart() async {
    print('checking for auto start');
    final upTime = await _fLauncherChannel.upTime();

    // if (upTime > Duration(minutes: 1).inMilliseconds) return;

    final query = _database.apps.select()
      ..where((tbl) => _database.apps.packageName.equals('com.kanout.mawaqit'));
    final apps = await query.get();

    if (apps.isNotEmpty) {
      launchApp(apps.single);
    }
  }
}
